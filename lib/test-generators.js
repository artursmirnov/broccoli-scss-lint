...
function testGenerator(callback) {
  return function(relativePath, errors, results) {
    ...
    let messages = `${relativePath} should pass sass-lint`;
...
function mochaString(relativePath, passed, messages) {
  let output = 'describe(\'SCSS Lint | ' + jsStringEscape(relativePath) + '\', function() {\n' +
...
const qunit = testGenerator(qunitString);
const mocha = testGenerator(mochaString);

module.exports = { quint, mocha };
